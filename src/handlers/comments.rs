use crate::{
    errors::{AppError, ErrorResponse},
    models::{AppState, Comment, CreateComment},
    utils::{check_delete_result, created_response},
    validation::ValidatedJson,
};
use axum::{
    extract::{Path, State},
    http::StatusCode,
    response::IntoResponse,
    Json,
};

#[utoipa::path(
    get,
    path = "/posts/{id}/comments",
    params(("id" = u64, Path, description = "文章 ID")),
    responses(
        (status = 200, description = "列出文章的所有评论", body = [Comment])
    ),
    tag = "Comments"
)]
pub async fn get_comments_for_post(
    State(state): State<AppState>,
    Path(id): Path<u64>,
) -> Result<Json<Vec<Comment>>, AppError> {
    let comments = sqlx::query_as::<_, Comment>("SELECT * FROM comments WHERE post_id = ?")
        .bind(id as i64)
        .fetch_all(&state.pool)
        .await?;
    Ok(Json(comments))
}

#[utoipa::path(
    post,
    path = "/posts/{id}/comments",
    params(("id" = u64, Path, description = "文章 ID")),
    request_body = CreateComment,
    responses(
        (status = 201, description = "成功创建评论", body = Comment)
    ),
    tag = "Comments"
)]
pub async fn create_comment_for_post(
    State(state): State<AppState>,
    Path(post_id): Path<u64>,
    json_payload: Json<CreateComment>,
) -> Result<impl IntoResponse, AppError> {
    let payload = json_payload.validate_json()?;
    let comment = sqlx::query_as::<_, Comment>(
        "INSERT INTO comments (post_id, author, content) VALUES (?, ?, ?) RETURNING *",
    )
    .bind(post_id as i64)
    .bind(&payload.author)
    .bind(&payload.content)
    .fetch_one(&state.pool)
    .await?;
    Ok(created_response(comment))
}

#[utoipa::path(
    put,
    path = "/posts/{post_id}/comments/{comment_id}",
    params(
        ("post_id" = u64, Path, description = "文章 ID"),
        ("comment_id" = u64, Path, description = "评论 ID")
    ),
    request_body = CreateComment,
    responses(
        (status = 200, description = "成功更新评论", body = Comment),
        (status = 404, description = "未找到评论", body = ErrorResponse)
    ),
    tag = "Comments"
)]
pub async fn update_comment(
    State(state): State<AppState>,
    Path((post_id, comment_id)): Path<(u64, u64)>,
    json_payload: Json<CreateComment>,
) -> Result<Json<Comment>, AppError> {
    let payload = json_payload.validate_json()?;
    let comment = sqlx::query_as::<_, Comment>(
        "UPDATE comments SET author = ?, content = ? WHERE id = ? AND post_id = ? RETURNING *",
    )
    .bind(&payload.author)
    .bind(&payload.content)
    .bind(comment_id as i64)
    .bind(post_id as i64)
    .fetch_one(&state.pool)
    .await?;
    Ok(Json(comment))
}

#[utoipa::path(
    delete,
    path = "/posts/{post_id}/comments/{comment_id}",
    params(
        ("post_id" = u64, Path, description = "文章 ID"),
        ("comment_id" = u64, Path, description = "评论 ID")
    ),
    responses(
        (status = 204, description = "成功删除评论"),
        (status = 404, description = "未找到评论", body = ErrorResponse)
    ),
    tag = "Comments"
)]
pub async fn delete_comment(
    State(state): State<AppState>,
    Path((post_id, comment_id)): Path<(u64, u64)>,
) -> Result<StatusCode, AppError> {
    let result = sqlx::query("DELETE FROM comments WHERE id = ? AND post_id = ?")
        .bind(comment_id as i64)
        .bind(post_id as i64)
        .execute(&state.pool)
        .await?;
    check_delete_result(result, "Comment")
}
